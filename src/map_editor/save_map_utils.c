...
/*   Updated: 2022/06/14 15:25:28 by shoogenb      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_rects(t_data *data, double scale, mlx_image_t	*menu_img)
{
	...
	cancel_rect.pos0.x += 60 * 4;
	cancel_rect.pos1.x = txt_rect.pos1.x;
	draw_rect(data, box_rect, 0x424242ff);
	draw_rect(data, txt_rect, 0x636363ff);
	draw_rect(data, save_rect, 0x636363ff);
	draw_rect(data, cancel_rect, 0x636363ff);
	pos.x = save_rect.pos0.x;
	pos.y = save_rect.pos0.y - scale * 0.67;
	draw_str(data, "accept", pos, scale, menu_img);
	pos.x = cancel_rect.pos0.x;
	draw_str(data, "cancel", pos, scale, menu_img);
	pos.y = box_rect.pos0.y;
	pos.x = box_rect.pos0.x + 20 * scale * 1.3;
	draw_str(data, "filename", pos, scale * 1.3, menu_img);
	draw_str(data, data->menu.filenname, txt_rect.pos0, scale * 0.67, menu_img);
}
...
void	filename_key_handler(struct mlx_key_data keys, void *param)
{
	...
	data = (t_data *)param;
	if (data->menu.reset_filename)
	{
		i = 0;
		data->menu.reset_filename = false;
	}
